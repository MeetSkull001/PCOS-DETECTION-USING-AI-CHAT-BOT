...
def main():
    ...
    while True:
        input_text = input("Enter a question (type 'quit' or 'exit' to stop): ").strip()

        if input_text.lower() in ['quit', 'exit']:
            print("Exiting the program.")
            break

        best_match = find_best_category(input_text)

        if best_match:
            print("Best Matching Category:", best_match["CATEGORY"])
            print("Similarity Score:", best_match["SIMILARITY_SCORE"])
            print("Matching Example Question:", best_match["MATCHING_QUESTION"])
        else:
            print("Out Of Bound")
